function result = Parron_Model_1( model_type, T, simu_num, lag_max)
warning('off')
...
for simu_num_count = 1 : simu_num
    ...
    for Tb1 = 2 : T - breaks_num
        ...
        if isreal(t_stat) ~= 1
            cValue_table(Tb1, 1) = nan;
            continue
        end
        
        cValue_table(Tb1, :) = t_stat(1:para_num);
    end
    
    a=find(cValue_table==min(cValue_table(find(cValue_table(:,1)~=0))));
    simu_result(simu_num_count,:) = cValue_table(a , : );
